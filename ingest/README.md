...
- **Universe layer** — `scripts/build_universe.py` parses GovInfo BILLSTATUS bulk zips (`--download` fetches them; keyless) into `data/universe/bills_{congress}.jsonl.gz`: one compact status record for every law-track bill of the covered Congresses. `scripts/fetch_universe_rollcalls.py` adds party-split roll calls (totals only). `scripts/fetch_popular_names.py` scrapes the OLRC Popular Name Table for the alias layer. The index build loads all of it automatically.
- **`billstatus.py`** — Keyless enrichment for any corpus bill: fetches GovInfo BILLSTATUS bulk data plus House Clerk / Senate LIS roll-call XML and writes one `billstatus.json` per bill dir. Carries the derived outcome (enacted / failed_cloture / died_in_committee / ...), every recorded vote with per-party splits and a bipartisan_support metric, the full action trail, and structured sponsor / cosponsors / subjects. Never touches metadata.json. Run via `scripts/backfill_billstatus.py` (all topics) or automatically by the seed fetchers.
