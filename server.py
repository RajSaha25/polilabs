...
class SummarizeRequest(BaseModel):
    """Ask for a grounded plain-language summary of ONE bill."""
...
# ---- bill summaries: grounded, cheap-model, cached (left-rail cards) ----
#
# A short plain-language summary of ONE bill, generated by a cheap model
# (Haiku) and grounded STRICTLY in the bill's own metadata + section table
# of contents (mechanically extracted — no body paraphrase is fed in, so the
# model can't drift far from the bill's structure). Kept OUT of the /chat
# loop so it never slows the main answer; the frontend auto-summarizes the
# top result and fetches the rest on demand. Summaries are LLM-generated, so
# the UI marks them as such and keeps them visually distinct from verbatim
# statute text (the anti-hallucination thesis). Cached per bill.

_summary_cache: dict[str, dict] = {}
_SUMMARY_MODEL = os.environ.get("POLILABS_SUMMARY_MODEL", "claude-haiku-4-5-20251001")
_SUMMARY_SYSTEM = (
    "You write a 2-4 sentence plain-language summary of ONE US federal bill "
    "for a busy legislative researcher, grounded STRICTLY in the metadata and "
    "section table of contents provided. State what the bill does and its main "
    "mechanisms or titles. Cite section numbers (e.g. 'Sec. 4') only when they "
    "appear in the material. You are given the table of contents, NOT the full "
    "body text — summarize at that level and never invent specific dollar "
    "amounts, dates, deadlines, or provisions that aren't shown. No filler and "
    "no throat-clearing — lead with the substance."
)


def _bill_grounding(bill: dict) -> str:
    """A compact, mechanical grounding doc from get_bill output: identity +
    top-level section headings. Bounded (get_bill returns only top-level
    sections), so even a huge bill stays cheap to summarize."""
    ident = bill.get("short_title") or bill.get("title") or bill.get("bill_id")
    lines = [f"Bill: {bill.get('bill_id')} — {ident}"]
    meta = []
    if bill.get("congress"):
        meta.append(f"{bill['congress']}th Congress")
    if bill.get("sponsor"):
        meta.append(f"sponsor {bill['sponsor']}")
    if bill.get("introduced_date"):
        meta.append(f"introduced {bill['introduced_date']}")
    if meta:
        lines.append("; ".join(meta))
    secs = bill.get("sections") or []
    if secs:
        lines.append("\nTable of contents (top-level sections):")
        for s in secs:
            label = s.get("canonical_citation") or s.get("section_id") or ""
            heading = s.get("heading") or ""
            lines.append(f"- {label}: {heading}".rstrip())
    return "\n".join(lines)


@app.post("/api/summary")
def api_summary(req: SummarizeRequest, _user: dict = Depends(require_user)) -> Any:
    """Grounded, cheap-model summary of one bill. Cached; login-only; charged
    to the caller's token budget like /chat (it spends model tokens)."""
    bill_id = (req.bill_id or "").strip()
    if not bill_id:
        raise HTTPException(422, "bill_id is required.")
    if bill_id in _summary_cache:
        return _summary_cache[bill_id]
    if not os.environ.get("ANTHROPIC_API_KEY"):
        return {"bill_id": bill_id, "summary": "",
                "error": "Summaries are not configured on the server."}
    if usage.is_over_limit(_user["id"], _user.get("email")):
        return {"bill_id": bill_id, "summary": "",
                "error": usage.limit_error_message(_user["email"])}
    ...
    try:
        ...
        msg = client.messages.create(
            ...
            messages=[{"role": "user", "content": _bill_grounding(bill)}],
        )
        ...
        out = {"bill_id": bill_id, "summary": text, "model": _SUMMARY_MODEL,
               "grounded_on": "metadata + table of contents"}
        if text:
            _summary_cache[bill_id] = out   # cache successful summaries only
        return out
    except anthropic.APIError as e:
